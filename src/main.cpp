...
int main(int argc,  char** argv) {
    ...
    if (argc == 1)
        while (1) {
            ...
            if (ret == QMessageBox::Yes) {
                ...
                path = fh->getDb();
                if (!open(path)) {
                    eh->displayErr("Invalid database file, see previous errors. Aborting.");
                    continue;
                }
                break;
            }
    ...
    while (1) {
        ...
        if (choice == "help")
            ...
            if (br == 3)
                eh->displayErr("Invalid backup location.");
            else if (br == 17)
                eh->displayErr("Improper permissions for file. Please select a location where the current user has write permissions.");
            else
